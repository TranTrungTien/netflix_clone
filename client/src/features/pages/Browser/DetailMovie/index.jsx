...
const DetailMovie = (props) => {
  ...
  useEffect(() => {
    const fetchRating = async () => {
      try {
        const rate = await axiosConfig.get("movies/api/rating", {
          params: {
            id: param.MovieId,
          },
        });
  ...
  useEffect(() => {
    const fetchPoster = async () => {
      try {
        const pos = await axiosConfig.get("movies/api/poster", {
          params: {
            id: param.MovieId,
          },
        });
  ...
  const openTrailerMovie = () => {
    setOpenTrailer(true);
  };
  useOnClickOutside(ref, () => setOpenTrailer(false));
  return (
    <>
      <header className="overflow-hidden fixed top-0 left-0 w-full">
      ...
      <main id="detail" className="overflow-hidden relative">
        {openTrailer && <Trailer ref={ref} />}
        {rating || poster ? (
          <>
            <section className="md:px-5% flex justify-center items-center h-screen w-screen">
              <div className="w-full h-95%">
                <div className="absolute top-0 left-0 z-10 w-full pt-20 px-6 md:px-0 text-center md:h-full md:static md:flex flex-col md:w-1/3 text-white justify-center items-start md:space-y-5">
                  <div className="mb-5">
                    <h1 className="text-7xl font-black">
                      {rating ? rating.title : poster ? poster.title : ""}
                    </h1>
                  </div>
                  ...
                  <div className="space-x-2">
                    ...
                    <span>{rating ? rating.imDb : "No Data"}</span>
                  </div>
                  ...
                  <button
                    className="py-2 px-7 border border-red-700 bg-red-E5 rounded-md font-semibold"
                    onClick={openTrailerMovie}
                  >
                    TRAILER
                  </button>
                </div>
    ...
    url(${
      poster && poster.posters
        ? poster.posters.link
        : "https://i1.wp.com/www.ecommerce-nation.com/wp-content/uploads/2018/10/404-error.jpg?fit=800%2C600&ssl=1"
    })`,
                      backgroundPosition: "top center",
                      backgroundRepeat: "no-repeat",
                      backgroundSize: "cover",
                    }}
                    className="absolute top-0 right-0 w-full h-screen z-negative blur"
                  ></div>
                </div>
              </div>
            </section>
            <section className="bg-181818 space-y-4 md:px-5%">
              <div className=" ">
                <h4 className="text-white text-2xl font-bold mb-3">Cast</h4>
                <Cast id={param.MovieId} />
              </div>
              <div className="text-gray-200 text-base">
                <h3>Content </h3>
                <Content id={param.MovieId} />
              </div>
              <div className="py-10 shadow-md px-3">
                <h3 className="text-xl font-semibold text-gray-100 pb-5">
                  Comments
                </h3>
                <div className="bg-gray-600 rounded-md p-5 space-y-1">
                  <div className="flex justify-start items-center space-x-3 shadow-sm">
                    ...
                    <div className="flex-1">
                      <input
                        ...
                        className="px-4 w-full h-full rounded-full bg-transparent focus: border-none focus:outline-none"
                      />
                    </div>
                  ...
                  <div className="flex justify-start items-center space-x-3 shadow-sm">
                    ...
                    <div>
                      <span className="text-base font-semibold">Mikami :</span>
                    </div>
                    <div className="flex-1">
                      <span>Greatest movies</span>
                    </div>
                    <div className="text-white">
                      <i className="fas fa-ellipsis-h cursor-pointer"></i>
                    </div>
            ...
            <section className="bg-181818 px-2% pt-20">
          ...
          <div className="min-h-screen bg-181818 text-center flex justify-center items-center text-2xl">
            <p>Wait !!!!!!!!!!!!!!</p>
          </div>
        )}
      </main>
